...
- Discrete blocks: **`.wp-block-query-filter-filter-checkboxes`**, **`.wp-block-query-filter-filter-radio`**, **`.wp-block-query-filter-filter-dropdown`**. Sort: **`.wp-block-query-filter-filter-sort`**.
- Container URL: filter keys = comma-separated slugs; plus **`search`**, **`pg`**, **`frel`**.
